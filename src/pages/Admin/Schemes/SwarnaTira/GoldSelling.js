    ...
    const toggle = (tab) => {
      if (tab == 1) {
        const filternonapprovedadata = data.filter(
          (item) => item.status_id === 3 &&  item.type_id ===2
        );
        ...
        const filterapprovedadata = data.filter((item) => item.status_id === 2 &&  item.type_id ===2);
        setUserData1(filterapprovedadata);
      }
