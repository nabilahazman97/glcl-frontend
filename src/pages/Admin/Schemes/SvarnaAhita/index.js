      ...
      .then((res) => {
        if (res.status == "404") {
          ...
          let filteredData1 = res.data.result.filter(
            (item) =>
                item.status_id === 3 && item.loan_type===7
        );
  ...
  const toggle = (tab) => {
    if (tab == 1) {
      const filternonapprovedadata = data.filter(
        (item) =>  item.status_id === 3 && item.loan_type===7
      );
      ...
      const filterapprovedadata = data.filter((item) => item.status_id === 2 && item.loan_type===7);
      setUserData1(filterapprovedadata);
    }
...
GoldCoinPawnList.propTypes = {
  preGlobalFilteredRows: PropTypes.any,
};
export default GoldCoinPawnList;
